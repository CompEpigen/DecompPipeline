...
x <- read.xlsx("/scratch/divanshu/ovarian/samplesheet.xlsx",1)
colnames(x) <- c("number", "sample_id","barcode")
rnb.set<-rnb.execute.import(data.source=list("/scratch/divanshu/ovarian/idats", x), data.type="idat.dir")
...
res<-prepare_data(
  RNB_SET=rnb.set, 
  WORK_DIR="/scratch/divanshu/ovarian/temp",
  DATASET="adgbbSorted",
  DATA_SUBSET="frontal",
...
saveRDS(rnb.set.filtered,"scratch/divanshu/newset/rnb.set.filtered.rds")
...
md.res<-start_medecom_analysis(
  ...
  WORK_DIR="/scratch/divanshu/ovarian/medecomrun",
  cg_groups=row20k,
  Ks=2:15,
