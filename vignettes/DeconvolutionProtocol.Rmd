...
author: "Michael Scherer, Petr Nazarov, Reka Toth, Shashwat Sahay, Tony Kamoa, Valentin Maurer, Christoph Plass, Thomas Lengauer, Joern Walter, and Pavlo Lutsik"
...
BiocManager::install(c("RnBeads","RnBeads.mm10","RnBeads.hg38"),dependencies=TRUE)
...
**BREAKPOINT** We provide an example report containing the processed RnBSet object for further analysis. It can be obtained from ()[] or directly loaded via:
...
**BREAKPOINT** We provide a list of CpGs that are selected for the downstream analysis. You can directly select those sites from you RnBSet object.
...
**BREAKPOINT** In case the analysis does not finish successfully, we provide the resulting ```FactorViz_outputs``` from ()[]. It can be directly loaded into your R session and further explored.
