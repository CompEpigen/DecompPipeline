...
match.meta.clin <- match(
	clinical.data$submitter_id,
		substr(meta.info$Comment..TCGA.Barcode.,1,12)
)
anno.frame <- na.omit(
	data.frame(
		clinical.data,
        	meta.info[match.meta.clin,]
	)
)
anno.frame$barcode <- unlist(
	lapply(
		lapply(
			as.character(anno.frame$Array.Data.File),
	            	function(x)strsplit(x,"_") 
		),
        	function(x) paste(x[[1]][1],x[[1]][2],sep = "_")
	)
)

anno.frame$Sentrix_ID <- sub("_.*$","",anno.frame$barcode)
anno.frame$Sentrix_Position <- sub("^[^_]+_","",anno.frame$barcode) 

write.table(
anno.frame,"annotation/sample_annotation.tsv",
