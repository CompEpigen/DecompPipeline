prepare_data<-function(
 ...
 #   annot<-rnb.annotation2data.frame(annotation(rnb.set))
    annot <- annotation(rnb.set)
    qf<-1:nrow(annot)
    
    if(beads){
      b.raw<-x="ANY", y="ANY"(rnb.set, row.names=TRUE)
    ...
    if(intensity){
      ...
      hm450_ann<-readRDS(file.path("/home/guptad/divanshu/pipeline", "hm450_cg_annot.RDS"))
